...
/*inline HMODULE LoadLibrary(String^ Filename) {
	typedef HMODULE(WINAPI *funcLoadLibraryW)(const wchar_t* lpFileName);

	funcLoadLibraryW LoadLibraryW = (funcLoadLibraryW)GetProcAddress("kernel32.dll", "LoadLibraryW");
	if (LoadLibraryW == NULL) return NULL;
	return LoadLibraryW(Filename->Data());
}*/
